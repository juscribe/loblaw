...
module Loblaw
  module LayoutHelper
    ...
    def get_canvas_class
      %(container _C_#{controller_name} _A_#{action_name})
    end

    def page_header(opts = {}, &block)
      content_tag :header, class: 'page-header' do
        opts.key?(:partial) ? render(opts.slice(:partial)) : capture(&block)
      end
    end
    ...
    #     make_section(partial: 'loblaw/shared/mini_feed') <<
    #     (opts.key?(:partial) ? render(opts.slice(:partial)) : capture(&block))
    #   end
    # end
    ...
    # # This won't be quite right, due to namespaceå
